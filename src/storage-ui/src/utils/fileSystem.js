...
export const AddEntry = (data, newEntry) => {
  ...
  const id = js_sha256.sha256(newEntry.path + newEntry.type);
  data[id] = newEntry;
...
export const showPathEntries = (parentPath, fileSystem) => {
  return fileSystem[js_sha256.sha256(parentPath + FOLDER)]
    ? fileSystem[js_sha256.sha256(parentPath + FOLDER)].children.map(
